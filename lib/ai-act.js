const AI_ACT_SOURCE_URL = "https://op.europa.eu/o/opportal-service/download-handler?identifier=dc8116a1-3fe6-11ef-865a-01aa75ed71a1&format=xhtml&language=en&productionSystem=cellar&part=";
...
async function fetchOfficialAiActMarkdown() {
  const response = await fetch(AI_ACT_SOURCE_URL);
  if (!response.ok) {
    throw new Error(`Failed to fetch AI Act source (${response.status})`);
  }
  ...
  if (
    ...
    throw new Error("Official source did not return the regulation text.");
  }
  ...
  return {
    ...
    sourceTextUrl: AI_ACT_SOURCE_URL,
  };
}
